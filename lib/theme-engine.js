...
(() => {
  ...
  // restyles claude.ai (and vice versa) — the two selections are independent,
  // exactly like the popup presents them.
  const SITES = ["claude", "chatgpt"];
  const selectedKeyFor = (site) => (site === "chatgpt" ? "cctThemeGpt" : "cctTheme");
  const siteOfTheme = (t) => (t && t.site === "chatgpt" ? "chatgpt" : "claude");
  ...
  function makeTheme(partial = {}) {
    ...
    const theme = {
      ...
      site: partial.site === "chatgpt" ? "chatgpt" : "claude",
  ...
  function exportable(theme) {
    return {
      ...
      tokens: theme.tokens,
      css: theme.css || "",
      // Claude is the historical default, so only the non-default site is
      // written out — older builds ignore the field entirely.
      ...(theme.site === "chatgpt" ? { site: "chatgpt" } : {}),
